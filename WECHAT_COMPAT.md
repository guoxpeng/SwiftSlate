...
| `app/src/preview/res/values/strings.xml`（+ `values-zh`、`values-zh-rCN`） | 应用名与无障碍服务名（`app_name` / `accessibility_service_label` = `SwiftSlate 微信适配`），以及两个兼容服务的显示名（`SwiftSlate 微信适配` / `SwiftSlate 微信适配（备选）`）。 |
...
2. 设置 → 无障碍 → 同时启用「SwiftSlate 微信适配」与「SwiftSlate 微信适配」
